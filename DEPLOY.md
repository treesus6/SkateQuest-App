...
3) Netlify CLI (for local dev + deploy)
   ...
   - Install and login:
    ...
    caches.keys().then(keys => Promise.all(keys.map(k => caches.delete(k))).then(() => console.log('Caches cleared')));
    ```

2) Verify manifest icons are present and valid from your server (run from project root):
